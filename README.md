...
- 左侧“资源”位于“媒体”下方，提供本地资源的搜索、类型/来源/文件夹筛选、收藏和静态封面卡片。没有封面的本地资源保持为清晰列表，不伪造动态图预览。
- 浏览器预览使用模拟资源数据，自动化测试已覆盖缓存、来源状态和界面；真实 AE 中的目录发现、扫描、文件夹选择及后续执行动作仍需单独宿主验收。脚本执行、预设应用、表达式写入、临时预览渲染、云端目录与多版本自动发现均未实现。
